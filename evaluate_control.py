...
def main():
    ...
    save_root = Path(f'/srv/local/---/results/{args.model}' + ('' if args.mask_type == 'gaussian1d' else f'_{args.mask_type}') + ('' if args.center_fraction == 0.08 else f'_CF_0{str(args.center_fraction)[2:]}') + ('' if args.acc_factor == 4 else f'_AF_{args.acc_factor}') + ('' if args.N == 2000 else f'_N_{args.N}') + ('_FS' if fat_suppression else '') + ('_brain' if brain else '') + '_no_norm')

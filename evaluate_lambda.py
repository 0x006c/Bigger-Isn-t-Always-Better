...
def main():
    ...
    elif args.mask_type == 'regular':
        mask = torch.zeros((320, 320))
        mask[:, ::args.acc_factor] = 1
        AC = int(args.center_fraction * 320)
        mask[:, (160 - AC // 2):(161 + AC // 2)] = 1
    ...
    save_root = Path(f'./lambdas/{args.model}' + ('' if args.mask_type == 'gaussian1d' else f'_{args.mask_type}') + (f'_{args.cutout}' if args.mask_type == 'outer' else '') + ('' if args.lam == 1 else f'_lam_{args.lam}') + ('' if args.center_fraction == 0.08 else f'_CF_0{str(args.center_fraction)[2:]}') + ('' if args.acc_factor == 4 else f'_AF_{args.acc_factor}') + ('' if args.N == 2000 else f'_N_{args.N}') + ('_FS' if fat_suppression else '') + ('_brain' if brain else '') + '_no_norm_flipped')
    ...
    if args.lam == 1:
        ...
        pc_fouriercs = get_pc_fouriercs_lambda(sde,
                                               predictor, corrector,
                                               inverse_scaler,
                                               snr=snr,
                                               n_steps=m,
                                               probability_flow=probability_flow,
                                               continuous=config.training.continuous,
                                               denoise=True, lambda_value=args.lam)
    
    for i, img in enumerate(test_dl):
        # if not i in [5, 56, 417, 419]:
        #     continue
        ...
        plt.imsave(str(save_root) + f'/under_{i}.png', under_img.squeeze().cpu().detach().numpy(), cmap='gray')
        plt.imsave(str(save_root) + f'/under_{i}_lim.png', under_img.squeeze().cpu().detach().numpy(), cmap='gray', vmin=0, vmax=1)
        if i > 8:
            break
...
def create_argparser():
    ...
    parser.add_argument('--mask_type', type=str, help='which mask to use for retrospective undersampling.'
                        ...
                        choices=['gaussian1d', 'uniform1d', 'gaussian2d', 'poisson', 'radial', 'outer', 'regular'])
    ...
    parser.add_argument('--lam', type=float, help='lambda value', default=1.0)

...
def main():
    ...
    avg_corpd /= 500
    imageio.v2.imwrite('/srv/local/---/figures/avg_corpd.png', normalize(avg_corpd[::-1, :]))
    ...
    avg_corpdfs /= 500
    imageio.v2.imwrite('/srv/local/---/figures/avg_corpdfs.png', normalize(avg_corpdfs[::-1, :]))
    ...
    avg_brain /= 500
    imageio.v2.imwrite('/srv/local/---/figures/avg_brain.png', normalize(avg_brain[::-1, :]))
    ...
    for i in range(30000):
        img = np.float64(np.transpose(imageio.imread(f'/srv/local/---/CelebA-HQ-img/{i}.jpg'), (2, 0, 1))) / 255.
        avg_celeba += img
    ...
    avg_celeba /= 30000
    imageio.v2.imwrite('/srv/local/---/figures/avg_celeba.png', np.transpose(normalize(avg_celeba), (1, 2, 0)))
    ...
    for i, (grad_x, grad_y) in enumerate(zip([grad_x_corpd, grad_x_corpdfs, grad_x_brain, grad_x_celeba], [grad_y_corpd, grad_y_corpdfs, grad_y_brain, grad_y_celeba])):
        ...
        np.savetxt(f'/srv/local/---/figures/hist_y_{i}.csv', np.stack((bins_y[mask_y][1:-1], hist_y[1:-1]), axis=1), delimiter=',')
        np.savetxt(f'/srv/local/---/figures/hist_x_{i}.csv', np.stack((bins_x[mask_x][1:-1], hist_x[1:-1]), axis=1), delimiter=',')

    #plt.xlabel(r'$\sqrt{( \nabla_x I )^2 + ( \nabla_y I )^2}$')
    #plt.ylabel('occurrences')
    plt.legend(('CORPD', 'CORPDFS', 'Brain', 'CelebA'))
    plt.tight_layout()
    plt.savefig('/srv/local/---/figures/hist.pdf')
    plt.close()
